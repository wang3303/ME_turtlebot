...
class AStar(object):
    ...
    def get_neighbors(self, state):
        res = []
        x, y = state
        delta = [-self.resolution, .0, self.resolution]

        for dx in delta:
            for dy in delta:
                if not (dx is 0 and dy is 0):
                    neighbor = self.snap_to_grid((x + dx, y + dy))
    ...
    def solve(self):
        while len(self.open_set) > 0:
            ...
            self.closed_set.append(cur)

            for next in self.get_neighbors(cur):
                if next in self.closed_set:
                    continue
                next_score = self.g_score[cur] + self.distance(cur, next)
                if next not in self.open_set:
                    self.open_set.append(next)
                elif next_score > self.g_score[next]:
                    continue
                self.came_from[next] = cur
                self.g_score[next] = next_score
                self.f_score[next] = next_score + self.distance(next, self.x_goal)
        return False
