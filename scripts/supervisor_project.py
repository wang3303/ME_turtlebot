...
class Supervisor:
	...
	# delivery address
        ...
        rospy.Subscriber('/detector/pizza', DetectedObject, self.vendor_detected_callback)
        rospy.Subscriber('/detector/cake', DetectedObject, self.vendor_detected_callback)
    ...
    def has_crossed_vendor(self):
        ...
        return (self.mode == Mode.VENDOR_CROSS and (rospy.get_rostime()-self.vendor_cross_start)>rospy.Duration.from_sec(VENDOR_CROSSING_TIME))
    ...
    def init_vendor(self):
        ...
        self.stop_start = rospy.get_rostime()
        self.stop_time = VENDOR_STOP_TIME
        self.mode = Mode.STOP
    ...
    def loop(self):
		...
		self.mode = Mode.STOP
            ...
            if len(self.goal_list) == NUM_VENDORS_TO_VISIT + 1:
            ...
            if self.close_to(self.x_g,self.y_g,self.theta_g):
                ...
                self.mode = Mode.WAIT_FOR_ORDERS

            else:
                self.nav_to_pose()

        elif self.mode == Mode.WAIT_FOR_ORDERS:
            rospy.loginfo("WAITING FOR ORDERS...")
            self.wait_for_orders_publisher.publish(True) #change this maybe
